  ...
  const handleSave = async () => {
    try {
      const url = editingProfile
        ? `${process.env.NEXT_PUBLIC_API_URL}/company-users/${editingProfile.id}`
        : `${process.env.NEXT_PUBLIC_API_URL}/company-users`;
  ...
  const handleDelete = async (id: number, isDefault: boolean) => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/company-users/${id}`,
        {
          method: 'DELETE',
          credentials: 'include',
        },
